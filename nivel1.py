...
def main():
    ...
    dir_fondo = "fondos/fondoNiveles/fondoNivel1.png"
    ...
    pygame.mixer.music.load("sonidos/songNivel1.mp3")  # Cargar soundtrack del menú
    ...
    dirFondoWin = "fondos/fondoWin.jpg"
    ...
    sonidoWin = pygame.mixer.Sound("sonidos/sonidoWin.mp3")
    sonidoWinPlay = False
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if time_left < 0 and nave.salud > 0:
                    x, y = pygame.mouse.get_pos()
                    if 209 <= x <= 486 and 389 <= y<= 408:
                        pygame.quit()
                        os.system("python3 nivel2.py")
                        sys.exit()
                    elif 199 <= x <= 502 and 445 <= y <= 464:
                        pygame.quit()
                        os.system("python3 menu.py")
        ...
        if not paused:  #Si el juego no esta pausado
                ...
                if nave.salud > 0:  #Si el jugador todavia tiene vida
                    ...
                    if 301 <= x <= 396 and 392 <= y <= 411:
                        if not select_sound:
                            select.play()
                            select_sound = True
                        pygame.draw.rect(screen, (238, 37, 201), (406, 398, 25, 7)) # Dibujar la línea rosada si el mouse está en el área de jugar  
                    elif 199 <= x <= 502 and 445 <= y <= 464:
                        if not select_sound:
                            select.play()
                            select_sound = True
                        pygame.draw.rect(screen, (238, 37, 201), (512, 448, 25, 7)) # Dibujar la línea rosada si el mouse está en el área de jugar
                    else:
                        select_sound = False
                    
            else:
                if nave.salud > 0:  #Si el tiempo se acabo y la nave todavia tiene vida, el jugador gano
                    nave.sonido_warning.stop()
                    ...
                    if 209 <= x <= 486 and 389 <= y<= 408:
                        if not select_sound:
                            select.play()
                            select_sound = True
                        pygame.draw.rect(screen, (238, 37, 201), (496, 395, 25, 7)) # Dibujar la línea rosada si el mouse está en el área de jugar  
                    elif 199 <= x <= 502 and 445 <= y <= 464:
                        if not select_sound:
                            select.play()
                            select_sound = True
        ...
        if paused and time_left > 0:    #Si el jugador no ha perdido y se presiona el boton de pausa
